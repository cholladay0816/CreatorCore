                ...
                <div class="hidden md:block">
                    @auth
                        <div class="ml-4 flex items-center md:ml-6">
                            <a href="{{ route('notifications.index') }}" class="relative bg-gray-800 p-1 rounded-full text-gray-400 hover:text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-white">
                                ...
                                @if(auth()->user()->notifications->whereNull('read_at')->count() > 0)
                                    <span class="absolute top-1 right-1 block h-2 w-2 rounded-full ring-2 ring-white bg-sky-400"></span>
                                @endif
                            </a>
                            <!-- Profile dropdown -->
                            <div class="ml-3 relative">
                                ...
                                <div @click.away="profile = false" style="display:none;" x-show="profile" class="z-20 origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg py-1 bg-white ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="user-menu">
                                    <a href="{{ route('profile.show') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100" role="menuitem">Your Profile</a>
                                    <a href="{{ route('creator.show', auth()->user()) }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100" role="menuitem">Your Creator Page</a>
            ...
            @auth()
                <div class="pt-4 pb-3 border-t border-gray-700">
                    <div class="flex items-center px-5">
                        ...
                        <a href="{{ route('notifications.index') }}" class="relative ml-auto bg-gray-800 flex-shrink-0 p-1 rounded-full text-gray-400 hover:text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-white">
                            <span class="sr-only">View notifications</span>
                            <!-- Heroicon name: outline/bell -->
                            <svg class="h-6 w-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
                            ...
                            @if(auth()->user()->notifications->whereNull('read_at')->count() > 0)
                                <span class="absolute top-2 right-2 block h-1 w-1 rounded-full ring-1 ring-white bg-sky-400"></span>
                            @endif
                        </a>
                    </div>
                    <div class="mt-3 px-2 space-y-1">
                        ...
                        <a href="{{ route('creator.show', auth()->user()) }}" class="block px-3 py-2 rounded-md text-base font-medium text-gray-400 hover:text-white hover:bg-gray-700">Your Creator Page</a>
